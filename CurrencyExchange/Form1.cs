    ...
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (i <= days)
            {
                ...
                double newRnd = rnd.NextDouble() * 12 - 6;
                v += newRnd + (Math.Pow(newRnd, 3) - 3 * newRnd) / 240;
                price = Math.Round(price * Math.Exp(m - Math.Pow(sig, 2) / 2) + sig * v, 3);
